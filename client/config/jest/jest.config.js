...
const userSetupFile = ['mjs', 'js', 'jsx', 'ts', 'tsx'].map((ext) => path.join(app.context, 'src', 'setupTests.' + ext)).find((file) => fs.existsSync(file));
...
module.exports = {
  ...
  testMatch: ['<rootDir>/**/__tests__/**/*.{js,jsx,ts,tsx}', '<rootDir>/**/*.+(spec|test).{js,jsx,ts,tsx}', '<rootDir>/**/*-specs.{js,jsx,ts,tsx}'],
  ...
  transformIgnorePatterns: ['[/\\\\]node_modules[/\\\\](?!@enact).+\\.(js|jsx|mjs|cjs|ts|tsx)$', '^.+\\.module\\.(css|less|sass|scss)$'],
  moduleNameMapper: {
