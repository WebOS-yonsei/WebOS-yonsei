    ...
    static: [
      {
        ...
        watch: {
          ...
          ignored: [ignoredFiles(path.resolve(app.context, 'src')), '/node_modules[\\/](?!@enact[\\/](?!.*node_modules))/'],
        },
      },
      {
...
function serve(config, host, port, open) {
  ...
  return choosePort(host, port).then((resolvedPort) => {
    ...
    const serverConfig = Object.assign({}, devServerConfig(host, resolvedPort, protocol, publicPath, proxyConfig, urls.lanUrlForConfig));
