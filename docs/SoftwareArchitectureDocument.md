...
- 데이터베이스에서 효율적으로 한 계정에 3명 이상 로그인한 경우에는 4번째 로그인을 허용하지 않기로 한 정책을 구현하는데 있어서 고민이 많았다. - 세션 테이블을 모두 순회하면서 해당 아이디에 유효한 세션 개수를 세는 방법도 있었지만 매우 비효율적이라고 판단해 별도로 현재 로그인된 사용자의 수를 세는 테이블을 두도록 토의했다.
