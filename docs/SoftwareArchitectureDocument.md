...
8. `FR08`: 영상 재생
   ...
   - `FR08-4`:
     - 미디어 재생 과정에서 Adaptive Streaming 기능을 지원한다.
...
9. `FR09`: 실시간 자원현황 조회
   ...
   - `FR09-2`
     - 실시간 자원현황 조회 요청을 받으면 현재 어플리케이션 사용현황에 대한 정보를 반환하고 이를 시각화 한다.
...
- 사용자 인증: 사용자가 로그인하면 Access Token이 발급되며, 이 토큰을 통해 사용자는 인증된 세션을 유지 할 수 있다.
...
- @enact/cli는 create-react-app(현재 cra)를 기반으로 만들어진 템플릿이다.
- 그런데 cra는 유지보수가 원활하게 되고 있지 않으며, 타 템플릿 대비 빌드타임이 오래 걸린다. 이런 부분은 프로젝트 개발진행에 있어 큰 불편함을 주었다.
- 하여 enact 템플릿에서 시간이 오래 걸리는 부분들의 성능을 개선하기로 했다.
- `npm run eject`를 통해 템플릿을 해체하고, 사용되지 않는 플러그인들을 삭제 및 성능이 안좋은 플러그인들을 더 나은 플러그인들로 교체했다.
- 대표적으로 babel-loader 등이 golang 기반의 esbuild-loader로 변경되었다.
- 플러그인 교체 및 템플릿 간소화 과정을 통해 빌드시간이 최소 70% 이상 감소하였다.

### 5.2 백엔드 로그인 동시접속 차단 구현

- 데이터베이스에서 효율적으로 한 계정에 3명 이상 로그인한 경우에는 4번째 로그인을 허용하지 않기로 한 정책을 구현하는데 있어서 성능상의 이슈가 있었다.
- 세션 테이블을 모두 순회하면서 해당 아이디에 유효한 세션 개수를 세는 방법도 있었지만 매우 비효율적이라고 판단해 별도로 현재 로그인된 사용자의 수를 세는 테이블을 두도록 토의했다.
- 비정규화를 통해 데이터 무결성 관리의 cost가 증가했지만 성능 개선의 benefit이 더 크다고 판단하였다.
